...
/* Tabla para señal cuadrada (12 muestras) con transiciones suaves */
static const int8_t square_table[12] =
...
{
    static int idx;                     /* avanza 1 muestra/segundo */
    int v = square_table[idx];
    idx = (idx + 1) % ARRAY_SIZE(square_table);
    return v;
}
